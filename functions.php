...
add_filter( 'gform_pre_render_' . HSD_REGISTRATION_FORM_ID, 'hsd_prepare_dynamic_display_name_field' );
add_filter( 'gform_pre_validation_' . HSD_REGISTRATION_FORM_ID, 'hsd_prepare_dynamic_display_name_field' );
add_action( 'gform_enqueue_scripts_' . HSD_REGISTRATION_FORM_ID, 'hsd_enqueue_dynamic_display_name_assets', 10, 2 );

function hsd_prepare_dynamic_display_name_field( $form ) {
	foreach ( $form['fields'] as &$field ) {
		if ( (int) $field->id === (int) HSD_FIELD_DISPLAY_NAME ) {
			if ( empty( $field->label ) ) {
				$field->label = __( 'Display Name', 'buddyboss-theme' );
			}

			if ( empty( $field->description ) ) {
				$field->description = __( 'This name will appear on your public profile.', 'buddyboss-theme' );
			}

			$field->cssClass .= ' hsd-dynamic-display-name-field';
		}
	}
...
function hsd_enqueue_dynamic_display_name_assets( $form, $is_ajax ) {
	$handle = 'hsd-gf-dynamic-label';
	$script_src = get_stylesheet_directory_uri() . '/assets/js/hsd-gf-dynamic-label.js';
	$script_ver = defined( 'WP_DEBUG' ) && WP_DEBUG ? filemtime( get_stylesheet_directory() . '/assets/js/hsd-gf-dynamic-label.js' ) : wp_get_theme()->get( 'Version' );
	...
	$config = array(
		'formId'         => (int) HSD_REGISTRATION_FORM_ID,
		'roleFieldId'    => (int) HSD_FIELD_USER_ROLE,
		'nameFieldId'    => (int) HSD_FIELD_DISPLAY_NAME,
		'defaultLabel'   => __( 'Display Name', 'buddyboss-theme' ),
		'defaultMessage' => __( 'This name will appear on your public profile.', 'buddyboss-theme' ),
		'labels'         => array(
			'fan'       => array(
				'label'       => __( 'Preferred Name', 'buddyboss-theme' ),
				'description' => __( 'This will show on your fan profile.', 'buddyboss-theme' ),
			),
			'supporter' => array(
				'label'       => __( 'Supporter Name', 'buddyboss-theme' ),
				'description' => __( 'Use your individual or organization supporter name.', 'buddyboss-theme' ),
			),
			'host'      => array(
				'label'       => __( 'Venue Name', 'buddyboss-theme' ),
				'description' => __( 'This is the public name fans will see for your venue.', 'buddyboss-theme' ),
			),
			'artist'    => array(
				'label'       => __( 'Artist / Stage Name', 'buddyboss-theme' ),
				'description' => __( 'Enter the name you perform under.', 'buddyboss-theme' ),
			),
		),
	);
